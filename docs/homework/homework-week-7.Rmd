---
title: "HW Week 7 - Introduction to Data Viz"
...
Next, replace year with status, and run the code:
...
Finally, group by both year and location; add both to your `group_by` function.
...
storms %>% 
  ...
  summarize(mean_wind = mean(wind)) %>% 
  ggplot(aes(x = year, y = mean_wind, fill = status)) +
  geom_col()
```
