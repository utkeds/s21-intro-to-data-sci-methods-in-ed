...
```{r}
download.file("https://github.com/making-data-science-count/s21-intro-to-data-sci-methods-in-ed/blob/main/data/tidykids.csv",
...
```{r}
tidykids_variable <- filter(tidykids, variable == "")
```
...
one of the above geometries. Recall that we add layers to ggplot2s (so, any line
...
Next, select a different geometry than the one you selected; choose which variable
...
Finally - for this portion of the homework - choose one more geometry 
(other than one you have already used), and create a plot using that geometry below:
...
focus on only one geometry for now, adding others over the next few weeks:
...
2. We are using a different geometry
