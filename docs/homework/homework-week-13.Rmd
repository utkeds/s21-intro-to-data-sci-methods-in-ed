...
Get these 3 files. Then use map() to apply the read_csv() function to each of them, or to be more precise, apply read_csv() to each of the file names, because that is what read_csv() takes as input. You can get the file names in more than one way, as discussed in class, the list.files() function can be a useful tool for this purpose, just keep in mind that read_csv() needs the rest of the file path that follows your working directory, i.e. what shows up if you do getwd().

You'll also notice that one of these files is missing its column names, so that is an additional complication you'll have to deal with. The read_csv() function has an argument to deal with this case, but you'll need to account for this when you use map(). One way could be to write your own function that deals with different cases that you would then pass to map(), but you could also just ignore column names for every file and put them in post hoc. There are surely other ways too, and it's your choice, you just need to end up with a list of data frames that each have correctly labeled columns.
