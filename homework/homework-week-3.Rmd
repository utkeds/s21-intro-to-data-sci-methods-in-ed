...
## Load the packages dplyr, readr, magrittr, forecats, and stringr. If you don't have them, install them first.
...
## Rename the columns using names of your choice in the dataset using a dplyr function
...
You can also set conditions based on specific values using the `==` operator. So in the previous example using `tvhours == 3`.

You can also use multiple conditions. Recall again last weeks homework, we asked you to add a second condtion to the filtering statement, something like `(tvhours > 3, age > 25)`. That statement would pick out rows meeting both conditions. You can also have multiple conditions that are mutually exclusive using the `|` operator. So we could have done something like `(tvhours > 3 | age > 25)` to filter rows meeting either of the two conditions. And you can keep chaining together with the `|` operator as many times as you need to. Why are we telling you all this? Oh no reason really.
...
The `mutate()` function in dplyr is used to add columns to datasets or modify existing ones. Type `?mutate` into the console to get more details. Here's an example of what that would look like:
...
The variable that was originally called `question_id` should be a numeric variable. Let's convert it into a factor. Base R has the `as.factor()` function, but let's use the similar `as_factor()` function from the forecats package. Make sure you loaded that at the beginning.
...
If you've been paying close attention, you may have noticed that there was a flaw in our data cleaning procedure for the "content" column. We got rid of any values outside of "1" "2" "3" "4" and "5", but there were some rows that had valid answers in them, but were filtered out by our filter, such as "4, Agree." We can see that should be a 4, but we need a way to catch these cases. 
...
```
In the example, we used the pattern "[123]{1}" which means "find one instance of any of the characters 1, 2, or 3 in each string" to extract those values.
...
str_extract(string = , pattern = "[123]{1}")
